/**
 * extract-scan — חילוץ פרטי משפחה מספח ת״ז דרך Gemini Vision.
 *
 * ⚠️ 16/09/2026: לא נקרא יותר מפאנל אנשי-הקשר. יוסף ביקש חינמי,
 * לא AI בתשלום — הוחלף ב-OCR מקומי (js/ocr-scan.js, Tesseract+PDF.js,
 * רץ לגמרי בדפדפן, בלי לשלוח כלום החוצה). נשאר פרוס, לא נמחק, למקרה
 * שיידרש שוב עבור דיוק גבוה יותר (ה-OCR החינמי רועש משמעותית על ספח
 * עם הרבה ילדים) — אבל שום קוד לא קורא לו כרגע.
 *
 * רק admin מחובר יכול לקרוא לזה — לא anon — כי Gemini Vision עולה
 * כסף, ובלי הבדיקה הזו כל מי שיש לו את ה-anon key הגלוי בקוד הדף
 * היה יכול להריץ אותו בחינם.
 *
